...
- **graph 调用重试**（`core/retry.py` + AgentLoopConfig 重试字段）：`ainvoke/astream` 异常后按**瞬时异常白名单**重试（默认 httpx 传输/超时 + `RetryableError` 标记；agent 层 config.py 注入 openai 限流/连接/超时/5xx 类），指数退避（默认 0.5/1/2s，`retry_max_attempts=3`）。**重试用 `input=None` 从 checkpoint 续跑**——失败的超步重执行、输入消息不重复追加（已实测验证）；首步即失败（无 checkpoint）时复用原输入。确定性错误不重试；耗尽后 invoke 抛异常、stream 出 error 事件。**注意分层**：openai SDK 对 5xx/429 自带约 3 次内部重试，会先吸收掉短故障，我们的层只在 SDK 耗尽后接管（E2E 实测：mock 连挂 5 次，SDK 消耗 4 次后我们的层重试 1 次成功）
