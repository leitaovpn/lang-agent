...
会话入口（repair/compress/retry/事件分类）的测试见 tests/test_agent/test_orchestration/test_session.py。
"""
import pytest
...
async def test_tools_node_passes_wrap_tool_call_from_context():
    """AgentContext.wrap_tool_call 存在时，默认 tools_node 将其传给 ToolNode。

    wrapper 拦截工具执行：本例验证 wrapper 收到 request 且可短路返回自定义
    ToolMessage（不执行真实工具）。
    """
    calls = []
    ...
    def wrapper(request, execute):
        ...
        return ToolMessage(
            content="wrapper 拦截结果", tool_call_id=request.tool_call["id"], name=request.tool_call["name"]
        )

    state_llm = FakeChatModel(
        responses=[
            AIMessage(content="", tool_calls=[TOOL_CALL]),
            AIMessage(content="计算完成"),
        ]
    )
    loop = AgentLoop(checkpointer=InMemorySaver())
    ctx = make_ctx(state_llm)
    ctx.wrap_tool_call = wrapper  # type: ignore[attr-defined]
    result = await run_graph(loop, "计算", ctx=ctx)
